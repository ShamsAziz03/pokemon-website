...
export function search(data: string, pokemonList: Pokemon[]) {
...
export async function getPokemon() {
	const randomId = Math.floor(Math.random() * 1025) + 1;
	const details = await Axios.get(
		`https://pokeapi.co/api/v2/pokemon/?limit=1&offset=${randomId}`,
	);
	return details.data.results[0];
}

export async function getPokemonsList() {
	const randomId = Math.floor(Math.random() * 1025) + 1;
	const details = await Axios.get(
		`https://pokeapi.co/api/v2/pokemon/?limit=6&offset=${randomId}`,
	);
	console.log(JSON.stringify(details.data.results, null, 2));
	return details.data.results;
}

export const pokemons = (searchText: string, pokemonList: Pokemon[]) => {
	if (!searchText) return pokemonList || [];
	return search(searchText, pokemonList);
};

export async function storePokemonOfDay() {
...
export function getPokemonOfDay() {
	...
	return (
		<>
			...
			<section className="grid grid-cols-[repeat(auto-fit,minmax(300px,1fr))] gap-20 p-10">
				...
				{pokemons(searchText, pokemonList)?.map((pokemon: Pokemon) => {
					return (
